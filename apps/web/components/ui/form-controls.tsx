...
export function FormMessage({
  ...
  return (
    <div
      className={`rounded-[calc(var(--radius-control)+0.15rem)] border px-4 py-3 text-sm leading-6 ${
        tone === 'info'
...
export function getTextControlClassName(hasError: boolean, tone: 'muted' | 'strong' = 'muted'): string {
  return `min-h-12 rounded-[var(--radius-control)] border px-4 py-3 text-sm text-foreground outline-none transition-colors placeholder:text-muted/70 ${
